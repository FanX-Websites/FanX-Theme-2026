<?php
/**
 * Git Widget for Admin Dashboard
 * Displays latest commits from the FanX Theme Github repository
 * 
 */

function df_reg_git_widget() {
	global $wp_meta_boxes;

	wp_add_dashboard_widget('widget_themeupdates', __('Git Repository Feed', 'df'), 'df_create_git_rss_box');
}
add_action('wp_dashboard_setup', 'df_reg_git_widget');

//RSS Feed
function df_create_git_rss_box() {
echo '<p><i>All updates to the FanX Theme code are logged here when the webdev uploads the code to the Github repository. Keep in mind that updates to the repository may not reflect the current status of the live site at all times.</i></p>';
	// Get RSS Feed(s)
